...
import rospy
import actionlib
from franka_gripper.msg import HomingAction, GraspAction, GraspGoal, GraspEpsilon, MoveAction, StopAction, HomingGoal, MoveGoal, StopGoal
...
class GripperCommander(object):
    ...
    def __init__(self, gripper_joint_names = ('panda_rightfinger', 'panda_leftfinger'), calibrate = False, **kwargs):
        """
        Constructor.
        """
        
        rospy.init_node('gripper_commander', anonymous=True)
        self.timeout = rospy.Duration(5)  # Adjust the timeout as needed
    ...
    def task_handler(self):
        ...
        if (self.command_queue):
            if (not self.free):
                ...
                command, percentage = self.command_queue[0]
                if (command):
                    self.open()
                else:
                    self.grasp(percentage)
    ...
    def gripper_commander_callback(self, msg):
        ...
        self.command_queue.append([command, percentage])
    ...
    def home_joints(self, wait_for_result = True):
        """
    ...
    def grasp(self, width, force = 10, speed = None, epsilon_inner = 0.005, epsilon_outer = 0.005,wait_for_result = True, cb = None):
        """
        Grasps an object.
